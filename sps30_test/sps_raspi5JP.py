#!/usr/bin/env python3
import csv
import os
import time
from datetime import datetime, timezone

import influxdb_client
from influxdb_client import Point
from influxdb_client.client.write_api import SYNCHRONOUS

from sensirion_shdlc_driver import ShdlcSerialPort
from sensirion_driver_adapters.shdlc_adapter.shdlc_channel import ShdlcChannel
from sensirion_uart_sps30.device import Sps30Device
from sensirion_uart_sps30.commands import OutputFormat

# -----------------------------
# SPS30 settings
# SPS30 センサー設定
# -----------------------------
UART_PORT = "/dev/ttyAMA0"      # Raspberry Pi側のUARTデバイス
BAUDRATE = 115200               # UART通信速度
SAMPLE_INTERVAL_SECONDS = 1     # 測定間隔（秒）

# -----------------------------
# InfluxDB settings
# InfluxDB 設定
# -----------------------------
# InfluxDB Configuration for Running locally
# ローカル環境で動かす場合のInfluxDB設定
# INFLUX_URL    = "http://192.168.188.141:8086"
# INFLUX_TOKEN  = "..."    # ← 必要に応じて置き換え


# InfluxDB Configuration for Running on a cloud
# InfluxDB Cloud用の設定
INFLUX_URL = "https://eu-central-1-1.aws.cloud2.influxdata.com"
INFLUX_TOKEN = "XwrLfZDuMGWQ9u2tl72X8X5gIMa-97SU5HLEfXizst_SAiubOwOTkrP5mTOTuZxpPbk2ALXa2UCET-dkmPnfJw=="
INFLUX_ORG = "free-tech"         # Organization名
INFLUX_BUCKET = "sps30_data"     # 保存先Bucket名
MEASUREMENT_NAME = "sps30"       # Measurement名

# -----------------------------
# Local file settings
# ローカルCSV保存設定
# -----------------------------
CSV_FILE = "sps30_log.csv"

# 後でfinally節で安全にcloseできるよう、あらかじめNoneで初期化
port = None
sensor = None
client = None
csv_file = None

try:
    # ---- Connect to SPS30 ----
    # SPS30へ接続
    port = ShdlcSerialPort(
        port=UART_PORT,
        baudrate=BAUDRATE,
        additional_response_time=0.02   # 応答待ち時間の追加
    )
    channel = ShdlcChannel(port)
    sensor = Sps30Device(channel)

    # センサー情報を取得
    serial_number = sensor.read_serial_number()
    product_type = sensor.read_product_type()

    print("Serial number:", serial_number)
    print("Product type :", product_type)

    # Stop previous measurement if already running
    # すでに測定中なら一度停止してから再開
    try:
        sensor.stop_measurement()
        time.sleep(0.5)
    except Exception:
        pass

    # 測定開始
    # OutputFormat(261) で測定フォーマットを指定
    sensor.start_measurement(OutputFormat(261))
    time.sleep(1)   # センサーが安定するまで少し待つ

    # ---- Connect to InfluxDB ----
    # InfluxDBへ接続
    client = influxdb_client.InfluxDBClient(
        url=INFLUX_URL,
        token=INFLUX_TOKEN,
        org=INFLUX_ORG
    )
    write_api = client.write_api(write_options=SYNCHRONOUS)

    # ---- Open local CSV file ----
    # ローカルCSVファイルを開く
    file_exists = os.path.exists(CSV_FILE)
    file_empty = (not file_exists) or os.path.getsize(CSV_FILE) == 0

    csv_file = open(CSV_FILE, "a", newline="")
    writer = csv.writer(csv_file)

    # ファイルが空ならヘッダーを書き込む
    if file_empty:
        writer.writerow([
            "timestamp_utc",
            "serial_number",
            "product_type",
            "pm1_0",
            "pm2_5",
            "pm4_0",
            "pm10",
            "nc0_5",
            "nc1_0",
            "nc2_5",
            "nc4_0",
            "nc10",
            "typical_particle_size_um"
        ])
        csv_file.flush()

    print("Measuring, writing to InfluxDB, and saving locally... Press Ctrl+C to stop.")
    # 測定開始メッセージ
    # InfluxDBへ送信しつつ、CSVにも保存する

    while True:
        # SPS30から測定値を取得
        values = sensor.read_measurement_values_uint16()
        (
            pm1, pm25, pm4, pm10,      # 質量濃度 [µg/m³]
            nc05, nc1, nc25, nc4, nc10,# 個数濃度 [#/cm³]
            tps                         # Typical Particle Size
        ) = values

        # 粒径を µm 単位へ変換
        tps_um = tps / 1000.0

        # 現在時刻（UTC）を取得
        now = datetime.now(timezone.utc)
        now_iso = now.isoformat()

        # ---- Write to InfluxDB ----
        # InfluxDBへ書き込むポイントを作成
        point = (
            Point(MEASUREMENT_NAME)
            .tag("sensor", "SPS30")
            .tag("serial_number", str(serial_number))
            .tag("product_type", str(product_type))
            .field("pm1_0", float(pm1))
            .field("pm2_5", float(pm25))
            .field("pm4_0", float(pm4))
            .field("pm10", float(pm10))
            .field("nc0_5", float(nc05))
            .field("nc1_0", float(nc1))
            .field("nc2_5", float(nc25))
            .field("nc4_0", float(nc4))
            .field("nc10", float(nc10))
            .field("typical_particle_size_um", float(tps_um))
            .time(now)   # タイムスタンプを設定
        )

        # データをInfluxDBへ送信
        write_api.write(bucket=INFLUX_BUCKET, org=INFLUX_ORG, record=point)

        # ---- Save locally to CSV ----
        # ローカルCSVへ保存
        writer.writerow([
            now_iso,
            serial_number,
            product_type,
            pm1,
            pm25,
            pm4,
            pm10,
            nc05,
            nc1,
            nc25,
            nc4,
            nc10,
            tps_um
        ])
        csv_file.flush()

        # ターミナルへ現在値を表示
        print(
            f"{now_iso} | "
            f"PM1.0={pm1} PM2.5={pm25} PM4.0={pm4} PM10={pm10} "
            f"TPS={tps_um:.3f} µm"
        )

        # 次の測定まで待機
        time.sleep(SAMPLE_INTERVAL_SECONDS)

except KeyboardInterrupt:
    # Ctrl+Cで停止した場合
    print("\n停止...")

finally:
    # 終了処理：測定停止、ポート解放、ファイルclose、DB接続close
    if sensor is not None:
        try:
            sensor.stop_measurement()
        except Exception:
            pass

    if port is not None:
        try:
            port.close()
        except Exception:
            pass

    if csv_file is not None:
        try:
            csv_file.close()
        except Exception:
            pass

    if client is not None:
        try:
            client.close()
        except Exception:
            pass
